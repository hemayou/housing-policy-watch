...
var rig = (function () {
  ...
  var M = {
    family: function (s) {
      ...
      var note = risk === 0 && !s.s5 ? '828 之前，你在为一个还不存在的房子还月供，而且是这条链上唯一没有抵押物的债权人。'
        ...
        : s.s4 ? '「能拿房、再还贷」，放款时点后移。项目本身仍可能烂尾，你只是不再替它垫资。'
        : s.s5 ? '月供轻了，总利息更重了。期限延长平滑的是现金流曲线，压不下购房总成本。'
        : '风险还在你这一端。';
    ...
    cfo: function (s) {
      ...
      var note = (s.s1 && s.s4 && !s.s3) ? '这是最难受的组合。回款全部推到竣工之后，资金封闭管理却还没到位。现金流缺口靠什么补，文件没有给答案。'
        : (s.s1 && s.s2) ? '两头同时收紧，买地要自己的钱，卖房要等房子盖完。高融资成本的房企在这一格里出局。'
        : (s.s1 && s.s3) ? '被拉长的销售周期，换来被拉长的贷款期限（7 年）和一条稳定的主办行通道。这是文件给的对冲。'
        : s.s2 ? '拿地杠杆归零。土地款要靠自有资金，行业能扩多快，从此看自己账上有多少钱。'
    ...
    mayor: function (s) {
      ...
      var note = s.s2 ? '「以地抵押→借款→买地→再抵押」的循环在缴款环节被掐断。抵押品先重估，再融资收缩随后；短期负债率会机械上行，因为分母骤缩而分子刚性。'
        ...
        : '这就是 2020 到 2022 年的那套办法，成交面积 −43.1%，地价 +11.0%。价格由行政与城投共同管理。';
  ...
  var NOTES = [
    { test: function (s) { return !s.s1 && !s.s2 && !s.s3 && !s.s4 && !s.s5; },
      t: '这是 828 之前的世界',
      b: '购房人的钱为开发建设融资，城投借钱托住地价，银行在链条中间做中转。三条线都通着，机器就能转，前提是地价还在涨。',
    ...
    { test: function (s) { return s.s1 && s.s4 && !s.s3; },
      t: '①④ 开、③ 关：房企的现金流断点',
      b: '销售端的回款被整体推到竣工之后，而资金封闭管理与主办银行通道还没建立。建设期的缺口只能靠开发贷或自有资金填，文件要求「实现开发贷款发放与项目建设周期相匹配」，说的就是这件事。少了③，保护购房人的两条款会直接变成对开发商的挤压。',
      c: '建房规〔2026〕3 号 三（二）；《商品住房开发贷款管理办法（试行）》第四条' },
    { test: function (s) { return s.s5 && !s.s1 && !s.s2 && !s.s3 && !s.s4; },
      t: '只拨⑤：唯一往回松的那一档',
      b: '五个开关里只有这一个是放松。期限 30→40 年、月所有债务偿债比 55%→60%。它降的是月供，总成本反而增加。居民部门已经连续 13 个季度贷款负增长，杠杆率从峰值回落约 6 个百分点，这个开关面对的是一张正在收缩的资产负债表，等着被释放的加杠杆意愿并不存在。',
      c: '《个人住房贷款管理办法（试行）》第十三条、第十条；NIFD 2026Q2' },
    { test: function (s) { return s.s1 && s.s2 && s.s3 && s.s4 && s.s5; },
      t: '五个全开：828 之后的世界',
      b: '风险沿着原来的链条被整体反向推回：家庭不再垫资，房企不再靠别人的钱扩张，地方政府失去土地金融的杠杆，银行成为承接方。方向上文献高度一致，都认为这是在拆病根。所有警告都集中在过渡期：化债置换是否按城投暴露定向分配、一般债与转移支付能否补位、地方支出效率跟不跟得上。',
      c: '见专题《地方债务：七篇文献》；Li & Kohl (2026) §7；王劲松等（2024）' },
    { test: function (s) { return s.s3 && !s.s1 && !s.s4; },
      t: '只拨③：把应急机制改成常态制度',
      b: '主办银行制首见于 2024 年住建部与金融监管总局的「白名单」，做法是单独建账、指定一家主办银行、防止多家银行同时抽贷。828 把它常态化了。单独看，它堵的是 2022 年那两个漏洞：钱没进监管账户，进了也被总包或集团挪走。',
      c: '《商品住房开发贷款管理办法（试行）》第七条、第八条；金融监管总局答记者问' }
  ];
  var DEFAULT_NOTE = { t: '继续拨', b: '每个开关旁边都写着它出自哪份文件的哪一条。试试把某一个单独关掉，看哪一本账最先变。这五件事看着并列，实际是一个整体，拆开任何一件，别处就会出现缺口。这台装置想说明的就是这个。', c: '' };
